...
LIDARServoManager::LIDARServoManager()
{
	...
	nh_.param<std::string>("LIDARServoManager/angle_command_topic", angle_command_topic, "lidar_servo_manager/angle_command_topic");
	angle_pub_ = nh_.advertise<std_msgs::Float32>(angle_command_topic, 1);
	std::string scanning_state_topic;
	nh_.param<std::string>("LIDARServoManager/scanning_state_topic", scanning_state_topic, "laser_stitcher/scanning_state");
	scanning_state_pub_ = nh_.advertise<std_msgs::Bool>(scanning_state_topic, 1);
	std::string angle_topic_in;
	nh_.param<std::string>("LIDARServoManager/angle_topic_in", angle_topic_in, "laser_stitcher/angle_topic_in");
	angle_input_ = nh_.subscribe<std_msgs::Float32>("laser_topic", 20, &LIDARServoManager::angleCallback, this);
	std::string service_name;
	nh_.param<std::string>("LIDARServoManager/service_name", service_name, "laser_stitcher/stationary_scan");
	ros::ServiceServer object_pose_server = nh_.advertiseService(service_name, &LIDARServoManager::stationaryScan, this);
...
bool LIDARServoManager::stationaryScan(laser_stitcher::stationary_scan::Request &req, laser_stitcher::stationary_scan::Response &res)
{
	...
	while(servo_angle_ + angle_step_	 < max_angle_ && ros::ok())
	{
		...
		ROS_DEBUG_STREAM("[LIDARServoManager] Sent a servo command... new angle: " << servo_angle_ << "; quat info: " << transform_from_parent.transform.rotation.z << " " << transform_from_parent.transform.rotation.w << ".");
	}
...
void LIDARServoManager::angleCallback(const std_msgs::Float32::ConstPtr& angle)
{
